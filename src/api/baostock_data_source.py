...
f"Baostock API error (Basic Info) for {code}: {rs.error_msg} (code: {rs.error_code})")
    ...
    def get_etf_list(self, date: Optional[str] = None) -> pd.DataFrame:
        """获取指定日期的ETF列表"""
        logger.info(f"Fetching ETF list for date={date or 'default'}")
        try:
            with baostock_login_context():
                # 首先获取所有股票列表
                rs = bs.query_all_stock(day=date)
                ...
                logger.info(
                    f"Retrieved {len(etf_df)} ETF records for date {date or 'default'}."
                )
                return etf_df

        except (LoginError, NoDataFoundError, DataSourceError, ValueError) as e:
            logger.warning(
                f"Caught known error fetching ETF list for date {date}: {type(e).__name__}"
            )
            raise e
        except Exception as e:
            logger.exception(
                f"Unexpected error fetching ETF list for date {date}: {e}"
            )
            raise DataSourceError(
                f"Unexpected error fetching ETF list for date {date}: {e}"
            )
    ...
    def get_etf_pe_data(self, date: Optional[str] = None) -> pd.DataFrame:
        """获取指定日期所有ETF的PE数据"""
        logger.info(f"Fetching ETF PE data for date={date or 'default'}")
        try:
            # 首先获取ETF列表
            etf_list = self.get_etf_list(date)
            all_pe_data = []
            ...
            # 遍历ETF列表，获取每个ETF的PE数据
            for _, row in etf_list.iterrows():
                code = row['code']
                try:
                    # 使用get_history_k_data_plus获取PE数据
                    pe_data = self.get_history_k_data_plus(
                        code=code,
                        start_date=current_date,
                        end_date=current_date,
                        fields=['code', 'pbMRQ', 'peTTM']  # 使用peTTM作为PE值
                    )

                    if not pe_data.empty:
                        # 添加ETF名称
                        pe_data['name'] = row.get('code_name', '')
                        all_pe_data.append(pe_data)
                except Exception as e:
                    logger.warning(f"Failed to get PE data for {code}: {e}")
                    # 继续处理下一个ETF，不中断整体流程
                    continue

            if not all_pe_data:
                logger.warning(f"No PE data found for any ETF on date {date or 'default'}")
                raise NoDataFoundError(f"No PE data found for any ETF on date {date or 'default'}")

            # 合并所有ETF的PE数据
            result_df = pd.concat(all_pe_data, ignore_index=True)
            logger.info(f"Retrieved PE data for {len(result_df)} ETFs")
            return result_df

        except (LoginError, NoDataFoundError, DataSourceError, ValueError) as e:
            logger.warning(f"Caught known error fetching ETF PE data: {type(e).__name__}")
            raise e
        except Exception as e:
            logger.exception(f"Unexpected error fetching ETF PE data: {e}")
            raise DataSourceError(f"Unexpected error fetching ETF PE data: {e}")
